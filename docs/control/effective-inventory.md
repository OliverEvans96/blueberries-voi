...
The project uses a single, unit-level freshness coordinate $f$ end-to-end: ground truth, the particle filter's internal state, the belief wire, and the ordering policy all speak $f$ directly, with no Weibull survival model on the production hot path. Once unit state is $f$ itself, $\mathbb{E}[f]$ is the direct sufficient statistic for the ordering rule — computing effective inventory any other way would mean tracking a second, disconnected notion of age alongside the one the filter actually maintains.

**Honest caveat.** Using $w(f_k) = f_k$ is exact only in the deterministic-decrement case. The production model's actual in-store aging is a **stochastic gamma** process, not a deterministic one, and under stochastic aging $w(f)$ is only *first-order* correct as $w(f) \approx f$ — Jensen's-inequality and first-passage-time effects mean the true expected-remaining-life fraction isn't perfectly linear in $f$ in general. The shipped version uses $w(f_k) = f_k$ anyway, because it matches the deterministic-gamma physics exactly, needs no extra numerics, and stays close under the actual stochastic dynamics. A more exact $w(f_k)$, computed via Monte Carlo simulation or a first-passage-time approximation on the actual gamma aging parameters, is a possible future refinement, not shipped code.
