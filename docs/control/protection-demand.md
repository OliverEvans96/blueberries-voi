...
The ordering rule (see [The ordering rule](/control/ordering-rule)) needs a target: how much total demand should today's order be able to cover before the next order arrives? We want a high quantile of that total demand — a value demand only rarely exceeds, not just the average. That target quantile is written $F^{-1}_{D}(\alpha)$, the $\alpha$-quantile of demand summed over the **protection window** — the days between now and the next time the store can place another order and have it arrive. This page is about computing that quantile once the delivery calendar makes the window's length, and the demand within it, vary from one order day to the next.
...
Once the protection window's length is fixed, the next question is: how much demand should the order defend against? Not the *average* demand over that window — that would leave the store short about half the time. Instead, the target is a high **quantile** of total demand: the $\alpha$-quantile (default $\alpha = 0.9$), chosen so demand exceeding the order is a relatively rare event, not a coin flip. When the average daily demand is the same on every day in the window, this quantile of the *sum* has a clean closed-form answer — a formula you can compute directly, with no simulation needed. When the calendar makes some days busier than others within the same window, there's no clean formula for the sum of those mismatched days, so the quantile is estimated by simulation instead.

## The math

Demand on a single day is modelled as a negative binomial distribution: mean $\mu$, and a variance-to-mean ratio $v > 1$ that captures how much more spread out day-to-day demand is than a plain Poisson process (a ratio of 1 would mean no extra spread). The protection window covers $n$ days starting at day $t$; total protection demand is $D = \sum_{i=0}^{n-1} D_{t+i}$.

**Flat-mean case (closed form).** If every day in the window has the same mean $\mu$, the sum of independent and identically distributed (i.i.d.) negative binomials is itself negative binomial, and its quantile has a direct formula: with $r = \mu / (v - 1)$ per day, the summed shape parameter is $r_{\text{sum}} = r \cdot n$ and success probability $p = r / (r + \mu)$, giving

$$
F^{-1}_{D}(\alpha) = \text{NB-PPF}(\alpha;\, r_{\text{sum}},\, p)
$$

Here "NB-PPF" just means the negative binomial's percent-point function (PPF) — the inverse of its cumulative distribution function — read off directly to get the quantile.

**Calendar-varying case (Monte Carlo).** When the calendar profile makes the daily means $\mu_{t}, \mu_{t+1}, \dots, \mu_{t+n-1}$ differ from each other, the sum of non-identical negative binomials has no simple closed-form quantile. The model falls back to simulation: draw many independent samples of each day's demand from its own negative binomial, sum them per draw, and read off the $\alpha$-quantile by sorting the simulated totals and picking the value at the right rank — **20,000** draws by default. The random draws are deterministically seeded from the order day, protection-window length, and $\alpha$ itself, so the same planning inputs always reproduce the same quantile, run to run.
...
The Monte Carlo fallback is a pragmatic choice rather than a mathematically derived one. Deriving, or even closely approximating, a closed-form quantile for a sum of non-identical negative binomials is hard enough that it isn't worth the effort here, so the model simulates its way to an answer instead. Fixing the random seed matters for a different reason: it keeps results comparable across runs, so that when two scenarios differ only in the information available to them, the difference in the outcome reflects that difference and not fresh random noise from a new draw.
...
- The negative-binomial demand model itself — including the overdispersion ratio that sets how much more variable demand is than a plain Poisson process — is an assumption about the *shape* of day-to-day demand variation, not something this page independently validates; days with very different demand dynamics from what the negative binomial captures would not be represented faithfully in $F^{-1}_D(\alpha)$.
