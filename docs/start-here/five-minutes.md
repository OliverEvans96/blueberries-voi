...
physics, the filter, the ordering rule, the Value of Information (VOI) number — is
just that, repeated, with uncertainty layered on top. This page walks through one
day in plain words, in the order the code runs it. No equations yet; those come
once the pieces have names.
...
**1. Every unit on the shelf loses a little freshness.** Each punnet loses a small,
random amount of freshness, drawn independently per punnet from a gamma process
(a smooth decay curve), faster on average when the store is warmer. Two punnets
that started the day identical can end it slightly different.

**2. Any unit whose freshness has hit zero is marked spoiled.** A unit can lose
freshness all the way down to 0 and become waste. There's no separate spoilage
roll — a unit spoils exactly when its freshness reaches zero during step 1, and
it's removed from the sellable pool immediately.
...
range — where units are close to spoiling — small timing choices change the
outcome. Whether spoilage is checked before or after sales, and whether delivery
lands before or after sales, shifts how much gets sold versus wasted. So the order
is fixed: freshness loss first, then resolve what's sellable, then sell, then
deliver last. The simulator and the belief-tracking filter use this exact same
order — if they didn't, the filter would be modeling a different store than the one
actually running.

Spoilage is folded directly into the freshness-loss draw: a unit spoils the moment
its continuous freshness loss reaches zero, before sales are drawn, not after.
There is no separate spoilage check applied afterward.

## In the code

| Step | What happens | File : line |
| --- | --- | --- |
| 1. Freshness loss | Each alive unit gets an independent random freshness loss, faster when warmer | `crates/voi_core/src/day_step.rs:269` (`apply_gamma_step`), `crates/voi_core/src/physics.rs:252` ([`apply_gamma_aging_independent`](/api/rust/voi_core/physics/fn.apply_gamma_aging_independent.html)) |
...
freshness loss typically is, how "prefer fresher" is weighted, and how a
delivery's starting freshness is chosen are each covered on their own pages with
the numbers attached. This page also doesn't cover what the store gets to see: a
unit spoiling or selling doesn't mean anyone running the store observed it happen.
What's observable depends on the observation scenario, covered under "What the
store can see."
