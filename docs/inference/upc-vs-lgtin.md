...
clamshell," or a per-lot LGTIN code that also says *which delivery lot* that clamshell came
...
three lots, not multiplied, so per-day filter runtime stays flat while LGTIN can finally tell
...
the *same* quantity at finer resolution. That's the core idea: **LGTIN refines UPC's
...
summing the LGTIN terms back up recovers the UPC term exactly.
...
So under **LGTIN** the filter births **three segments**, each from its own
...
Beyond refinement of pooled totals, LGTIN now buys three things UPC structurally cannot
...
2. **Composition.** Under LGTIN the bag is exactly 13/13/14 units per lot; under UPC it is
...
Why LGTIN can never be *less* informative than UPC: let $f$ be a particle's full
freshness state. The LGTIN spoilage term factors the pooled Poisson-binomial PMF along
lot boundaries,
...
gate. So LGTIN's per-stage terms are a refinement of UPC's, evaluated on the identical
state — LGTIN never discards anything UPC had. The one place the table above shows an
extra row for LGTIN with nothing to sum from on the UPC side is cross-lot allocation:
UPC's sales total says nothing about *how* it split across lots, so there is no UPC term
to refine there — that's the new information LGTIN alone supplies.

**Lot matching is by identity, not position.** A LGTIN observation names lots by id;
...
delivery identity is exactly what LGTIN's serialized code provides that UPC's pooled code
doesn't. Falling back to aggregate scoring on an unmatched lot id, rather than killing
every particle, keeps LGTIN's worst case no worse than UPC's — a single misaligned
...
LGTIN's per-lot terms are scored with closed-form Poisson-binomial and multinomial terms
rather than single-sample Monte Carlo draws. Scoring with single samples would make
LGTIN's *estimated* variance compound with lot count, so on paper LGTIN could look *less*
...
against comparable metrics in this codebase's diagnostics, not a proof that LGTIN's
...
(they sum to the observed pooled total); the code does not separately reconcile a LGTIN
feed against an independently reported UPC feed if a real deployment somehow supplied both
and they disagreed. And the unmatched-lot fallback means a LGTIN feed with persistently
