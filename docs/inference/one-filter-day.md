...
resample, birth — on every particle in the bank, whether the store scans a pooled
Universal Product Code (UPC), which identifies the product only, or a per-lot LGTIN — a
Global Trade Item Number (GTIN) paired with a batch/lot number, which identifies one
specific delivery batch of that product rather than the product in general. This page
walks through those four stages once, in order, so the other inference pages can point
back at it instead of re-deriving the mechanics.
...
UPC, one lot's worth for the LGTIN per-lot term).

**Spoilage — the per-unit death probability.** The store's daily gamma decrement law —
the random, one-directional freshness loss drawn for the day, never bouncing back up —
is shared across units, same shape $k$ and scale $\theta$, but every live unit draws its
...
where $F_\Gamma$ is the decrement's cumulative distribution function (CDF) — the
probability the decrement is at or below a given value. These $p_i$ are independent but
*not* identical — a nearly-spoiled unit ($f_i$ small) has a much higher $p_i$ than a
fresh one.
...
distribution — a calculation that accounts for every unit's own, slightly different
chance of having spoiled, rather than assuming they're all equally likely. Its
probability mass function (PMF) has no closed form but is exact and cheap to compute via
a forward dynamic program (DP):
...
death count matches, the filter runs the same dynamic program (DP) backward to sample
exactly which units died, conditioned on the observed count $w$ actually being right.
This is a *fully adapted proposal* — a candidate way of generating which units died,
produced before it's weighted against the day's evidence, that's constructed to already
match the target distribution exactly. Surviving units then draw their decrement from
the same gamma law but *truncated* to $[0, f_i)$, so they land just short of spoiling
instead of risking a draw that would have killed them. Because the proposal already
matches the target exactly, the resulting importance weight reduces to the
Poisson-binomial PMF above — nothing extra to correct for.
...
count could only reject particles whose lots were ordered wrong, not sharpen the belief
about freshness *level*.
...
sampling with scoring. It would also make LGTIN — despite observing *more* — pay a
variance penalty for every extra lot it scored separately, making its beliefs *more*
diffuse than UPC's on the same data. Splitting the sales likelihood into a deterministic
closed-form term (the multinomial allocation above) and a separate, unscored
state-transition removal avoids that.

**Caveat:** the Poisson-binomial dynamic program costs $O(n \cdot w)$ per lot segment per
particle per day. That's affordable at the particle and unit counts this project runs
at, and it scales with both the day's death count and the segment size.
...
| Poisson-binomial log-PMF (dynamic program) | $\log P(W=w)$ | `crates/voi_core/src/unit_ll.rs:27` ([`pb_log_pmf`](/api/rust/voi_core/unit_ll/fn.pb_log_pmf.html)) |
...
| Unscored without-replacement (WOR) removal draw | — | `crates/voi_core/src/unit_ll.rs:260` ([`sequential_kernel_path_logprob`](/api/rust/voi_core/unit_ll/fn.sequential_kernel_path_logprob.html)) |
...
only ever reweights *which whole-store hypothesis* to keep. The death set and the sales
removal are themselves random draws, not the true underlying identities. Because of
that, two particles that agree on every observable count can still disagree about
exactly *which* physical units were sold or spoiled — that's an irreducible
identifiability gap the per-unit grid does not resolve.
