...
**Studio freshness accuracy uses $W_1$ on this wire, not the raw particles.** The interactive studio's belief-accuracy "Distribution" cell scores 1-Wasserstein distance between the rebinned wire histogram and truth live-unit freshness (see `web/src/charts/beliefAccuracy.ts`). All-days is the mean of those per-day $W_1$ values. Count CRPS needs the particle predictive $\{N^{(i)}\}$, which this wire does not export — only $\mathbb{E}[N]$ via `lot_counts` — so the studio does not claim a count-CRPS score from the flattened payload.
