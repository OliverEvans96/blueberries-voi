...
The [knowledge ladder](/ladder/rungs) only matters if richer observation channels
actually produce a more accurate belief about shelf freshness. This page measures that
directly: replay the same 30 days of deliveries and customer demand through six
observation rungs and see how far each rung's belief drifts from the truth the simulator
generated.
...
you get one number per rung: how far off, on average, that rung's belief runs from the
truth.
...
The measurement is designed so the only thing changing between rungs is what the filter
is allowed to see — same truck deliveries, same customer demand, same order sequence,
same random seeds, replayed once per rung. The regression test that encodes this,
`crates/voi_core/tests/t150_phase2_arrival_model.rs::ac2_11a_empirical_ladder_tracking_mae`,
checks the ordering empirically against simulated ground truth: it asserts
$\mathrm{MAE}(F3) < \mathrm{MAE}(F2) < \mathrm{MAE}(P0)$ strictly, and fails if the ladder
ever goes flat. A purely algebraic guard — one that only checks arithmetic relationships
...
**Alternative considered:** using the residual-spread gap between rungs (e.g. requiring
$\mathrm{sd}(f \mid F3) < \mathrm{sd}(f \mid F2)$ at a tight tolerance) as the guard
instead. The F2→F3 gap in residual spread is small by construction — temperature
contributes only about 1.6% of the variance a pack date leaves unresolved (see
...
separates the rungs more cleanly.
...
resolve at small lot counts — at 8 units per delivery the noise floor swamps the P0→F2
signal. The numbers above are the cross-seed average from the notebook that generated
them; treat the third decimal digit as noise, not signal.
...
- **Waste totals alone are not reliably more accurate than books-only on this path.** P1
  ...
  freshness. Don't read this page as "any extra data helps" — it plainly doesn't, here.
