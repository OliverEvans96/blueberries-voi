...
1. **Age.** For every alive unit ($f_i > 0$), draw an independent decrement and apply it: $f_i \leftarrow \max(f_i - \Delta_i,\ 0)$, with $\Delta_i \sim \mathrm{Gamma}(k\bar\phi, \theta)$ (see [how fruit ages](/store/gamma-aging)).
