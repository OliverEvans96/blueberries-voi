"""T-008 Stage 0: sequential_wor_pmf age likelihood (exact joint vs mean-field).

Hand helpers below lock the ADR 0086 contract against production
...
def _hand_log_p_sales_waste_given_ages(
    ...
    """Contract lock: sequential WOR sales x independent binomial waste (ADR 0086)."""
...
def test_rbpf_soft_stub_still_has_sales_pow_marker() -> None:
    backends_path = (
    ...
    assert "sales_pow" in body
    assert "waste_pow" in body
...
def test_adr_0049_and_0057_status_still_accepted() -> None:
    ...
    for name in (
        ...
        # STATUS line must remain ACCEPTED (no flip in T-008).
        status_lines = [
