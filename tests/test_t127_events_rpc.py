...
def test_events_does_not_advance_session_day() -> None:
    _rpc("init", {"seed": 1})
    _rpc("step", {"order": 0})
    snap_before = _rpc("init", {"seed": 1})
    day_before = snap_before["result"]["episode_day"]
    _rpc("step", {"order": 0})
    _rpc("events", {"since_day": 0})
    snap_after = _rpc("init", {"seed": 1})
    _rpc("step", {"order": 0})
    # Re-init to get comparable state — events call must not step.
    out = _rpc("events", {"since_day": 0})
    assert out.get("ok") is True
