...
def test_day_step_injected_smoke(monkeypatch: pytest.MonkeyPatch) -> None:
    """Injected demand path: Rust ``day_step_injected`` vs Python reference."""
    counts_in = [20, 15]
    taus_in = [1.0, 3.0]
    lot_ids_in = [1, 2]
    demand = 12
    ...
    assert math.isclose(
        float(py_result.sales_total), float(rs_sales), abs_tol=_STRUCTURAL_ATOL
    )
    py_final = sum(c.n for c in py_result.cohorts)
    rs_final = sum(int(n) for n in rs_counts)
    assert py_final >= 0 and rs_final >= 0


def test_filter_step_one_day_smoke(monkeypatch: pytest.MonkeyPatch) -> None:
    """One-day filter update: normalized weights from Rust and Python paths."""
    ...
    rbpf = RBPF(params=ModelParams(), N=n, K=2, L=2)
    rng = np.random.default_rng(_FILTER_SEED)
    rbpf.initialize(rng, L=2)
    assert rbpf._state is not None
    summary = rbpf.step(P1Obs(sales, waste, 0), rng)
    py_weights = [float(w) for w in rbpf._state.weights]
    assert len(py_weights) == n
    assert all(w >= 0.0 for w in py_weights)
    assert math.isclose(sum(py_weights), 1.0, abs_tol=1e-9)
