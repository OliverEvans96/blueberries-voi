...
#[test]
fn lgtin_upc_diag_supports_shard_cli() {
    let diag_src = read_example("lgtin_upc_diag.rs");
    assert!(
        diag_src.contains("\"--shard\""),
        "lgtin_upc_diag must support --shard <regime_idx> <seed_idx>"
    );
    assert!(
        diag_src.contains("fn run_shard"),
        "lgtin_upc_diag must implement run_shard"
    );
}
...
fn shipments_thermal_phi_bar_non_degenerate() {
    ...
    assert!(
        ...
        "lgtin_upc_diag must define shipments_thermal()"
    );
