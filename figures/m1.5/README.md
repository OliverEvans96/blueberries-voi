...
| --- | --- | --- |
| `m15_stage_a_rung_map.png` | P0, P1, F1, F1s, F2a, F2 (all six) | Stage A prior vs posterior SD bar map |
...
```bash
uv sync --all-extras
uv run python -c "from blueberries_voi.viz.m15 import run_m15_stage_a; run_m15_stage_a()"
```

Short result MD: `experiments/m15_stage_a_result.md`.
...
| Figure | Rung(s) | FIL-11 role |
| --- | --- | --- |
| `m15_stage_b_P0_rank.png` | P0 | Stage B rank hist (diagnostic if A fail) |
| `m15_stage_b_P1_rank.png` | P1 | Stage B rank hist (diagnostic if A fail) |
| `m15_stage_b_F1_rank.png` | F1 | Stage B calibration ranks |
| `m15_stage_b_F1s_rank.png` | F1s | Stage B calibration ranks |
| `m15_stage_b_F2a_rank.png` | F2a | Stage B calibration ranks |
| `m15_stage_b_F2_rank.png` | F2 | Stage B calibration ranks |
| `m15_oracle_ladder_gap.png` | P1, F2 (default compare) | Oracle ladder vs B-state |
...
uv sync --all-extras
uv run python -c "
from blueberries_voi.viz.m15 import run_m15_stage_b, run_m15_oracle_ladder
run_m15_stage_b(
    root_seed=0,
    stage_a_pass={'F2a': True, 'F2': True},
    write_md=True,
)
run_m15_oracle_ladder(root_seed=0, write_figure=True)
"
```

Short result MD (coverage + diagnostic labels + gap table):
`experiments/m15_stage_b_result.md`.
...
| Artifact | Path |
| --- | --- |
| Stage C figure | `figures/m1.5/fil11_stage_c_generative.png` |
