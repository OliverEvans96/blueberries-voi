...
- Runnable experiment covers rungs **{P0, P1, F1, F1s, F2a, F2}** with shared
  `root_seed` → `tests/test_stage_a_multirung.py::test_run_m25_stage_a_exported`
  → `tests/test_stage_a_multirung.py::test_run_m25_stage_a_default_rungs_cover_six_scenarios`
  → `tests/test_stage_a_multirung.py::test_run_m25_stage_a_accepts_shared_root_seed`
  — currently failing: `run_m25_stage_a` not exported
- Prior vs posterior SD + contracted + tight-control + ≥5% margin +
  ...
  → `tests/test_stage_a_multirung.py::test_run_m25_stage_a_default_contraction_margin`
...
- Figures under `figures/m2.5/` + README map →
...
- No VOI dollars; no CTL →
  ...
  — currently failing: `viz.m25` / `viz.stage_a` missing
