...
Scope: T-015 / Wave 4 L-fallback for M2.5. Full-suite red from
T-016 / T-017 / T-018 (and unrelated M2.5 closeout process checks) is
...
- `uv run pytest -q` → exit 1, **36 failed, 150 passed, 1 skipped**, coverage 82.50% (≥80%); failures are T-016/T-017/T-018 RED + M2.5 closeout process (see below), not `test_l_fallback` / filter L-fallback
...
- `experiments/m25_l_remeasure.md` (M2.5 L remeasure + fallback note)
...
- [x] Short bakeoff / note addendum under `experiments/` documents re-measured L + fallback — verified by `test_m25_l_remeasure_experiment_note_documents_fallback` + file present
...
- **T-018 / M2.5 closeout process** — open RED QA without needs-human, missing reviews, changelog/DoD claims
- Unrelated closeout checkbox failures for older tickets (T-009, T-010, T-011, T-013) in `tests/test_m25_closeout.py`
