...
- [x] `.team/qa/` contains green records for M2.5 tickets in scope; no open red qa blockers without `needs-human` — verified: T-009–T-018 QA STATUS PASS; F2a documented needs-human
- [x] `.team/reviews/` contains APPROVED reviews for the M2.5 implementation waves — verified: T-009–T-010 combined + T-011–T-018 APPROVED; `.team/reviews/M2.5.md` DoD checklist all `[x]`
- [x] `.team/changelog.md` has a plain-English M2.5 entry — verified: `## 2026-08-12 — M2.5 filter complete…` covers rungs/obs, physics-honest LL, P0/P1/F2a caveats, F2 + oracle (T-018); left as-is (no rewrite)
- [x] DoD checklist from plan §9 copied into `.team/reviews/` with each item checked — verified: `.team/reviews/M2.5.md`
...
- None. F2a pack_date emit remains `needs-human` in backlog (honest, allowed; not an M2.5 DoD reopen).
