# T-018 test map (RED — M2.5 close-out DoD contracts)
...
- All of T-009–T-017 acceptance criteria are marked done in their specs (or
  ...
  → `tests/test_m25_closeout.py::test_m25_ticket_spec_acceptance_criteria_done_or_waived`
...
- `.team/qa/` contains green records for M2.5 tickets in scope; no open red qa
  blockers without `needs-human`
  → `tests/test_m25_closeout.py::test_m25_ticket_qa_record_green`
  → `tests/test_m25_closeout.py::test_no_open_red_qa_blockers_without_needs_human`
  — currently failing: T-016 RED without needs-human; T-017 missing qa record

- `.team/reviews/` contains APPROVED reviews for the M2.5 implementation waves
  (or equivalent per-ticket reviews covering T-009–T-017)
  → `tests/test_m25_closeout.py::test_m25_ticket_review_approved`
  — currently failing: T-016, T-017 (no subject review); earlier tickets with
  APPROVED reviews pass

- `.team/changelog.md` has a plain-English M2.5 entry (client voice): what the
  filter can observe per rung, that likelihood matches sim physics, and honest
  P0/P1 caveats if Stage A failed there
  → `tests/test_m25_closeout.py::test_changelog_has_m25_client_voice_entry`
  — currently failing: no M2.5 close-out entry yet
...
- DoD checklist from plan §9 is copied into `.team/reviews/` or a close-out note
  ...
  → `tests/test_m25_closeout.py::test_dod_checklist_copied_and_checked`
  — currently failing: no close-out DoD note yet
  → `tests/test_m25_closeout.py::test_plan_section_9_dod_still_present`
...
- No production CTL/VOI/browser modules landed under this milestone beyond
  pre-existing stubs
  → `tests/test_m25_closeout.py::test_no_production_ctl_voi_browser_under_m25`
  — currently passing (stubs empty)
  → `tests/test_m25_closeout.py::test_m25_milestone_claims_do_not_assert_ctl_voi_shipped`
  — currently failing: no M2.5 changelog/close-out text yet to assert non-claims against
...
- Whether changelog is one entry vs per-wave rollup — process preference; tests
  accept either an `## … M2.5` heading or M2.5-tagged bullets with required themes
...
- Review coverage is by filename subject (or M2.5/T-018 close-out spanning T-009–T-017),
