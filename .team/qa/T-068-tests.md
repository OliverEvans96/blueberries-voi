...
uv run pytest \
  ...
  tests/test_age_likelihood.py::test_production_particle_filter_update_uses_exact_wor_not_mc_ll \
  tests/test_particle_filter_unique_mf.py \
  tests/test_audit_t044_mf_stubs_hygiene.py::test_production_particle_filter_update_does_not_call_mean_field_update \
  tests/test_audit_t044_mf_stubs_hygiene.py::test_production_p1_does_not_invoke_mean_field_update \
  tests/test_mc_likelihood.py::test_production_particle_filter_update_does_not_default_to_observation_loglik_mc \
...
- No production closed-loop path calls `mean_field_update` (AST + P1 spy; thin
  callers clean)
  → `tests/test_arrival_only_count_filter.py::test_particle_filter_update_source_does_not_call_mean_field_update`
  — failing: `mean_field_update` still named in `_particle_filter_update`
  ...
  → `tests/test_production_mean_field.py::test_particle_filter_update_source_does_not_call_mean_field_update`
  ...
  → `tests/test_particle_filter_unique_mf.py::test_particle_filter_update_does_not_name_mean_field_update`
  — failing: MF still present
  → `tests/test_particle_filter_unique_mf.py::test_duplicate_particles_do_not_invoke_mean_field_update`
  — failing: MF still invoked
  → `tests/test_audit_t044_mf_stubs_hygiene.py::test_production_particle_filter_update_does_not_call_mean_field_update`
  — failing: MF still in `_particle_filter_update`
  → `tests/test_audit_t044_mf_stubs_hygiene.py::test_production_p1_does_not_invoke_mean_field_update`
  — failing: spy non-empty
...
- Default particle weights = exact sequential WOR; multinomial selectable
  → `tests/test_arrival_only_count_filter.py::test_particle_filter_update_weights_use_exact_wor_not_mc_default`
  ...
  → `tests/test_arrival_only_count_filter.py::test_particle_filter_sales_likelihood_field_defaults_to_exact_sequential_wor`
  — failing: `sales_likelihood` field missing on `ResearchParticleFilter`
  → `tests/test_arrival_only_count_filter.py::test_particle_filter_sales_likelihood_multinomial_selectable`
  ...
  → `tests/test_mc_likelihood.py::test_production_particle_filter_update_does_not_default_to_observation_loglik_mc`
  — failing: MC still named in `_particle_filter_update`

- Count transitions match `day_step` physics (not ±1 RW)
  → `tests/test_arrival_only_count_filter.py::test_particle_filter_update_has_no_pm1_count_random_walk`
  — failing: `rng.integers(-1, 2, …)` still present
  → `tests/test_arrival_only_count_filter.py::test_particle_filter_update_count_path_names_day_step_physics`
  — failing: no day_step / allocate_sales names in `_particle_filter_update`
  → `tests/test_production_mean_field.py::test_particle_filter_update_has_no_pm1_count_random_walk`
  — failing: same ±1 RW
...
- Guard supersessions (ADR 0105 contracts replace ADR 0091 locks)
  ...
  → `tests/test_particle_filter_unique_mf.py` unique-MF dedup retired — RED above
  ...
  — failing: choose_backend / ResearchParticleFilter still `mean_field`
  → `tests/test_m2_closeout.py::test_production_backend_is_not_age_mean_field`
  — failing: same constant
  → `tests/test_belief.py` helper `_stepped_production_particle_filter` now requires
...
- `mean_field_update` / MC LL remain importable off production path
  ...
  — currently **passing**
  → `tests/test_particle_filter_unique_mf.py::test_mean_field_update_still_importable_off_production_path`
  — currently **passing**
