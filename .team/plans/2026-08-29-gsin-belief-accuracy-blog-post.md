# 2026-08-29/30 — Decisions log + forward plan: GSIN belief-accuracy fix, blog post figures
...
| 2 | `DEFAULT_L_DIM`: 10 → **50** | Root-caused the GSIN-looks-worse-than-UPC anomaly to `belief_flat_from_unit_bank`'s fixed-`L` wire truncation, which hits GSIN far harder than UPC (GSIN fragments deliveries into more lot segments). Benchmarked `L=3,10,20,30`: no runtime cost difference among 10/20/30 under `damped_sw` or `sla_pb` (well inside 500ms/1000ms Studio budgets); `sla_pb` tail latency was worse at `L=3`. Chose 50 for headroom since the cost is effectively free. | `crates/voi_core/src/params.rs` |
