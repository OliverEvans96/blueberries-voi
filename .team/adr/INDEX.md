...
- Modal batch map (2026-08-23): provisional **[0145](./0145-modal-batch-map-optional-ops.md)** (T-155; used by T-150 notebook 13 closeout) — optional Modal / local ProcessPool drivers for nb13 and `lgtin_upc_diag`. Does **not** replace 0144.
- MOD-16 revisit — three fixed lots per delivery (2026-08-26): **[0149](./0149-mod-16-three-fixed-lots-per-delivery.md)** — `L = 3` fixed and known (not latent, not random); delivery quantity split not multiplied; LGTIN holds three segments, UPC holds one mixture-of-laws cohort; no new mask fields. **Supersedes [0038](./0038-mod-16-lots-per-delivery-below-the-scanning-rung.md)** (option A); Oliver reopened 0038 to authorize this.
- Cold-chain break events (2026-08-26): **[0150](./0150-arrival-thermal-break-events.md)** — Poisson-count / Exponential-duration break episodes plus transit generative v2 (bottom-up Abdella-matched stage durations, trip cool/nominal/warm mode, hourly OU on path) replace the truncated-normal mean transit temperature; temperature trace is the generative primitive for `Λ` via `resolve_arrival_exposure`; filter projection is mode-mixed stage-gamma baseline × Poisson–gamma breaks (no hourly filter latent); calibration guards are `Var(log d)`, clean-chain `φ̄` moments at `ρ → 0`, and generative/filter coherence — not the retired 98.4% duration-share figure. **Supersedes [0144](./0144-f-native-hierarchical-arrival-model.md)'s §3 transit-temperature clause and [0148](./0148-abdella-derived-arrival-fit.md)'s truncated-normal fit step**; rest of both ADRs unaffected. Baseline amended T-163.
...
| [0031](./0031-mod-09-demand-model.md) | `MOD-09` | Demand model (i.i.d.) | SUPERSEDED BY 0113 |
...
| [0112](./0112-x-11-mwf-delivery-base-case.md) | `X-11` / CAL-01 | MWF delivery base case (LT=1; order Sun/Tue/Thu) | ACCEPTED |
| [0113](./0113-mod-09-calendar-demand.md) | `MOD-09` / CAL-01 | Known NB with calendar DOW×week structure | ACCEPTED |
| [0114](./0114-order-schedule-api.md) | `CAL-A1` | OrderSchedule type/API (can_order / protection_days) | ACCEPTED |
...
| [0145](./0145-modal-batch-map-optional-ops.md) | *(ops)* / T-155 | Modal batch map for nb13 / lgtin_upc_diag (optional extra) | PROPOSED |
| [0149](./0149-mod-16-three-fixed-lots-per-delivery.md) | `MOD-16` | Three fixed lots per delivery; split not multiplied; LGTIN segments / UPC mixture | ACCEPTED |
| [0150](./0150-arrival-thermal-break-events.md) | MOD-11 / MOD-18 / MOD-19 / FIL | Cold-chain breaks + transit generative v2 (stages/modes/OU); filter mode×gamma×breaks | ACCEPTED |
