...
audit spec and mypy stubs only; F3 grid, shard output, and conditional lgtin mount
...
Notebook 13 (filter-accuracy channel factorial) and the `lgtin_upc_diag` harness each
...
4. Modal images install a **pre-built** `blueberries_voi` PyO3 wheel (`maturin build`)
   and copy the release `lgtin_upc_diag` example binary — **no Rust compile on cold
   start**.
5. `lgtin_upc_diag` gains a `--shard <regime_idx> <seed_idx>` mode so one truth episode
...
- **288 independent lgtin jobs (regime × seed × channel)** — recomputes truth 6× per
