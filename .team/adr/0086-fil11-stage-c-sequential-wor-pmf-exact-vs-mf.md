# 0086. Filter age likelihood for FIL-11 Stage C (`sequential_wor_pmf`) — exact joint vs mean-field check
...
4. Run **FIL-11 Stage C** as exact joint vs mean-field **induced joint** on **fixed count paths**
   (not a full RBPF-vs-RBPF bakeoff). Leave production soft `_rbpf_update` / RBPF unchanged until
   settle.
...
- **Wiring mean-field into production RBPF before settle** — rejected as premature; Stage C is
  evidence-only until gates and a settle note say otherwise.
...
**Locked in:** Stage C uses `sequential_wor_pmf` + exact joint vs MF on fixed counts; soft
`_rbpf_update` stays until a later ticket; ADR 0049 / 0057 statuses are unchanged by this work.
