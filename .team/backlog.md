...
- **Done — truth vs belief audit (T-115 / ADR 0125):** Landed on `main` (merge `3ddfd69`). Report [truth-vs-belief-audit.md](./reports/truth-vs-belief-audit.md). **Substantive remainder:** *Frontend knowledge-scenario UI audit* only. Optional polish: truth visual language, belief-grid axis gate, chip legend, `main.ts` integration test. ADR 0125 still `PROPOSED` — human accept when ready.
- **Optional — Rust backend truth/belief wire (T-110 follow-on):** PyO3 `py_delta`/`py_snapshot` emit empty `live_lots` and stub belief (`crates/voi_py/src/lib.rs`); `act_rollout` in `voi_core` orders from true counts not particle belief. Opt-in `BLUEBERRIES_VOI_BACKEND=rust` only; studio default Python/WASM unaffected. See audit report wire deep dive.
