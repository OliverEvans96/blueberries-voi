# LGTIN/UPC investigation data

Machine-generated inputs for `notebooks/14_lgtin_vs_upc_filter_accuracy.ipynb`.
...
|------|-------|----------------|------|
| `lgtin_upc_before.json` | `team/T-137/implement`, pre-ADR-0137 | binomial waste; LGTIN likelihood degenerate (the bug) | 18 |
| `lgtin_upc_pre_t141.json` | ADR 0137 | shared-δ interval constraint | 18 |
| `lgtin_upc_t140.json` | `team/T-140/implement`, ADR 0141 | shared-δ interval; unified gamma **arrival** | 24 |
| `lgtin_upc_after.json` | `team/T-141/implement`, ADR 0143 | **independent per-unit aging** + Poisson-binomial | 24 |

**T-150 (ADR 0144):** `voi_profits_after.json` was regenerated under damped base-stock
(`n_rollout_paths=0`). `lgtin_upc_after.json` was **not** regenerated (same T-141 spoilage
...
**`lgtin_upc_pre_t141.json` is not a run of the T-140 parent harness.** It is byte-identical
to the ADR 0137-era `lgtin_upc_after.json` (commit `a332d26`) — three regimes, no thermal
fixture, generated two tickets before T-140. The genuine T-140 baseline is
`lgtin_upc_t140.json`, regenerated at that tip with the corrected harness.

## Harness correction

The T-138 rewrite of `lgtin_upc_diag` reintroduced, in the **measurement** code, the fixed
...
`lgtin_upc_t140.json` and `lgtin_upc_after.json` here are both regenerated with the corrected
...
```bash
experiments/regen_lgtin_upc_data.sh                              # belief metrics (~17 min)
...
pre-ADR-0143 cost). `crates/voi_core/tests/lgtin_upc_ac12.rs` shells out to it, so both of
...
`lgtin_upc_before.json` predates the thermal fixture and carries only the first three, so the
