...
WEB = REPO_ROOT / "web"
OUTPUT = "studio-cockpit-annotated.png"
...
def _ensure_wasm() -> None:
    ...
    if not wasm_dir.is_dir() or not list(wasm_dir.glob("*.wasm")):
        subprocess.run(["./scripts/build-wasm.sh"], cwd=REPO_ROOT, check=True)
...
def _annotate(raw_path: Path, target_dir: Path) -> Path:
...
def render(out_dir: Path | None = None) -> Path:
    ...
    try:
        ...
        return _annotate(raw_path, target_dir)
    finally:
        _cleanup_vite()
